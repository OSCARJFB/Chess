...
bool pawnRules(int selectY, int selectX, int moveY, int moveX, std::vector <char> movement, char piece, bool playerTurn) {
    ...
    if (moveY == (selectY + 1) && (moveX == (selectX + 1) || moveX == (selectX - 1)) && piece == 'P' && playerTurn == true) {
        for (unsigned int i = 1; i < movement.size(); i++)
            if (movement[i] == ' ' && (movement[i] != 'W' && movement[i] != 'Q' && movement[i] != 'K' && movement[i] != 'R' && movement[i] != 'P' && movement[i] != 'B'))
                return false;
...
bool rookRules(int selectY, int selectX, int moveY, int moveX, std::vector <char> movement, char piece, bool playerTurn) {
    ...
    if (piece == 'R' && playerTurn == true) {
        if (selectX != moveX && selectY == moveY) {
            for (unsigned int i = 1; i <= movement.size(); i++) {
                if (i == (movement.size() - 1) && (movement[i] != 'W' && movement[i] != 'Q' && movement[i] != 'K' && movement[i] != 'R' && movement[i] != 'P' && movement[i] != 'B'))
                    return true;
                else if (movement[i] != ' ')
                    return false;
            }
...
bool checkChess(char board[8][8], bool playerTurn) {
    ...
    // Check if the threatend king is allowed to move anywhere. Remove king when doing this as to not cover any positions.
    board[kingY][kingX] = ' '; 
    if (chessMate == true) {
        for (int i = 0; i < 8; i++) 
            for (int n = 0; n < 8; n++)
    ...
    // Done add king back to the board. 
    board[kingY][kingX] = king;

    // if true, make sure that the spot of allowed movement is empty. 
    if (is_Safe[0] == true && board[kingY - 1][kingX + 1] == ' ' && board[kingY - 1][kingX + 1] != (queen || bishhop || knight || rook || pawn)) { chessMate = false;  std::cout << "1hello"; }
    if (is_Safe[1] == true && board[kingY][kingX + 1] == ' ' && board[kingY][kingX + 1] != (queen || bishhop || knight || rook || pawn)) { chessMate = false;  std::cout << "2hello"; }
    if (is_Safe[2] == true && board[kingY + 1][kingX + 1] == ' ' && board[kingY + 1][kingX + 1] != (queen || bishhop || knight || rook || pawn)) { chessMate = false;  std::cout << "3hello"; }
    if (is_Safe[3] == true && board[kingY + 1][kingX] == ' ' && board[kingY + 1][kingX] != (queen || bishhop || knight || rook || pawn)) { chessMate = false;  std::cout << "4hello"; }
    if (is_Safe[4] == true && board[kingY + 1][kingX - 1] == ' ' && board[kingY + 1][kingX - 1] != (queen || bishhop || knight || rook || pawn)) { chessMate = false;  std::cout << "5hello"; }
    if (is_Safe[5] == true && board[kingY][kingX - 1] == ' ' && board[kingY][kingX - 1] != (queen || bishhop || knight || rook || pawn)) { chessMate = false;  std::cout << "6hello"; }
    if (is_Safe[6] == true && board[kingY - 1][kingX - 1] == ' ' && board[kingY - 1][kingX - 1] != (queen || bishhop || knight || rook || pawn)) { chessMate = false;  std::cout << "7hello"; }
    if (is_Safe[7] == true && board[kingY - 1][kingX] == ' ' && board[kingY - 1][kingX] != (queen || bishhop || knight || rook || pawn)) { chessMate = false;  std::cout << "8hello"; }
