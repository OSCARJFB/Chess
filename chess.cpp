...
#define NO_SEL 65   // Value used if nothing is selected.
...
void drawBoard(SDL_Renderer*, SDL_Rect[64], char[8][8], int, int, bool, int, int, bool);
...
void AI(char[8][8], bool, bool, bool*, bool, int, int, int*, int*);
void AI_locateAllPieces(char[8][8], int[16][2], int[16][2], int*, int*);
void AI_kingFindThreat(char[8][8], int[16][2], int, int, int, int*, int*); 
bool AI_kingTryDestroyThreat(char[8][8], int, int, int[16][2], int, int*, int*, int*, int*);
bool AI_moveKing(char[8][8], int, int, int[16][2], int, int*, int*, int*, int*); 
bool AI_blockThreat(char[8][8], int, int, int, int, int[16][2], int, int*, int*, int*, int*);
void AI_makeMove(char[8][8], bool*, int, int, int, int);
bool AI_checkStatus(char[8][8], int, int, int[16][2], int); 
void AI_resetBoard(char board[8][8], char, char, int, int, int, int);
void AImarkSquares(int, int, int*);
...
int main(int argc, char* argv[]) {
    ...
    int selectY = NO_SEL, selectX = NO_SEL, moveY = NO_SEL, moveX = NO_SEL;                // Location of a selected piece and its movement target.
    int kingX = NO_SEL, kingY = NO_SEL;                                                    // Stores the x and y position of the king.
    int hoovering = NO_SEL, mouseX, mouseY, clicked = NO_SEL, clickedMove = NO_SEL;        // Mouse interaction, such as hoovering, position and click. 
    ...
    int aiSel = NO_SEL, aiMove = NO_SEL;                                                   // Will mark a move on the chess board. 
    ...
    // AI and rand.
    srand(time(NULL));           // For semi-random values. 
    bool AI_running = false;      // If true AI is running (currently turned of). 
	...
	while (is_Running == true) {
		...
		// Search for any threat to the king. 
        ...
        if (selected == true) {
            ...
            for (int i = 0; i < 64; i++) {
                ...
                if (mouseX <= rect_Board[i].x + width && mouseX >= rect_Board[i].x &&
                    ...
                    if (playerTurn == true && board[k][n] == 'R' && board[selectY][selectX] == 'W' && shortCastp1 == true && k == 0 && n == 7) {     // Shortcast.
                        can_move = true;
                        hoovering = i;
                        movement.clear();
                        break;
                    }   
                    if (playerTurn == false && board[k][n] == 'r' && board[selectY][selectX] == 'w' && shortCastp2 == true && k == 7 && n == 7) {    // Shortcast.
                        can_move = true;
                        hoovering = i;
                        movement.clear();
                        break;
                    }
                    if (playerTurn == true && board[k][n] == 'R' && board[selectY][selectX] == 'W' && longCastp1 == true && k == 0 && n == 0) {      // Longcast.
                        can_move = true;
                        hoovering = i;
                        movement.clear();
                        break;
                    }
                    if (playerTurn == false && board[k][n] == 'r' && board[selectY][selectX] == 'w' && longCastp2 == true && k == 7 && n == 0) {      // Longcast.
                        can_move = true;
                        hoovering = i;
                        movement.clear();
                        break;
        ...
        // AI action: Excutes only when AI is running. 
        if (AI_running == true && playerTurn == false) AI(board, shortCastp2, longCastp2, &playerTurn, check, kingY, kingX, &aiSel, &aiMove);
					...
					for (int i = 0; i < 64; i++) {
						...
						if (mouseX <= rect_Board[i].x + width && mouseX >= rect_Board[i].x &&
                            ...
                            if(playerTurn == true) // P1. 
								if ((board[k][n] == 'P' || board[k][n] == 'R' || board[k][n] == 'K' || board[k][n] == 'B' || board[k][n] == 'Q' || board[k][n] == 'W') && selected == false) {
                            ...
                            if (selected == true) {
                                ...
                                clicked = NO_SEL;           // Reset.
                                hoovering = NO_SEL;         // Reset.
        ...
        if ((shortCastp1 || longCastp1 || shortCastp2 || longCastp2) == true)  castlingAction(board, &playerTurn, &shortCastp1, &longCastp1, &shortCastp2, &longCastp2, selectX, selectY, moveX, moveY, check);

        // Draw graphics. 
		drawBackground(rend_Board);
		drawBoard(rend_Board, rect_Board, board, clicked, hoovering, can_move, aiSel, aiMove, AI_running);
...
bool moveAction(int selectY,      int  selectX, 
    ...
    int kingX = NO_SEL, kingY = NO_SEL;                             // King x and y position on the chess board.
...
bool pawnRules(int selectY, int selectX, 
    ...
    if (piece == 'P' && playerTurn == true) {
        ...
        if (moveY == selectY + 1 && (moveX == selectX + 1 || moveX == selectX - 1)) {       // Attack move.
            for (unsigned int i = 1; i <= movement.size(); i++) {
                if (i == movement.size() - 1 && movement[i] == 'w' || movement[i] == 'q' || movement[i] == 'k' || movement[i] == 'r' || movement[i] == 'p' || movement[i] == 'b') return true;
                else return false;
            }
        }
...
bool knightRules(int selectY, int selectX, 
    ...
    if (piece == 'K' && playerTurn == true)
        if (moveY == selectY + 2 && moveX == selectX + 1 ||
            ...
            moveY == selectY + 1 && moveX == selectX - 2 ||
            moveY == selectY - 1 && moveX == selectX - 2)
            if (target == ' ' || target == 'p' || target == 'r' || target == 'b' || target == 'w' || target == 'q' || target == 'k') return true;
...
bool bishopRules(int selectY, int selectX, 
    ...
    int stepsY = NO_SEL, stepsX = NO_SEL;
...
bool kingRules(int selectY, int selectX, 
    ...
    if (piece == 'W' && playerTurn == true) {
        if (selectY != moveY && (moveY == selectY + 1 || moveY == selectY - 1) && selectX == moveX) {
            for (unsigned int i = 1; i <= movement.size(); i++) {
                if (i == movement.size() - 1 && movement[i] != 'W' && movement[i] != 'Q' && movement[i] != 'K' && movement[i] != 'R' && movement[i] != 'P' && movement[i] != 'B') return true;
                else if (movement[i] != ' ') return false;
            }
        ...
        if (selectX != moveX && (moveX == selectX + 1 || moveX == selectX - 1) && selectY == moveY) {
            for (unsigned int i = 1; i <= movement.size(); i++) {
                if (i == movement.size() - 1 && movement[i] != 'W' && movement[i] != 'Q' && movement[i] != 'K' && movement[i] != 'R' && movement[i] != 'P' && movement[i] != 'B') return true;
                else if (movement[i] != ' ') return false;
            }
        ...
        if (stepsY == stepsX && (moveX == (selectX + 1) || moveX == (selectX - 1)) && (moveY == selectY + 1 || moveY == selectY - 1)) {
            for (unsigned int i = 1; i <= movement.size(); i++) {
                if (i == movement.size() - 1 && movement[i] != 'W' && movement[i] != 'Q' && movement[i] != 'K' && movement[i] != 'R' && movement[i] != 'P' && movement[i] != 'B') return true;
                else if (movement[i] != ' ') return false;
            }
...
bool is_check(char board[8][8], bool playerTurn) {
    ...
    // x and y position for the king, any possible threat and available dirusptors.
    int kingX = NO_SEL, kingY = NO_SEL; 
    int threatX = NO_SEL, threatY = NO_SEL; 
    int removeX = NO_SEL, removeY = NO_SEL;         
    ...
    for (int i = 0; i < 8; i++) {
        for (int n = 0; n < 8; n++)
            if (board[i][n] != ' ') {
                checkmate = lookForMove(board, i, n, kingY, kingX, board[i][n], opponent, king);
    ...
    // Is the threatend king is allowed to move anywhere? 
    ...
    if (checkmate == true) {
        for (int i = 0; i < 8; i++)
            for (int n = 0; n < 8; n++)
                if (board[i][n] != ' ') {
                    threat[0] = lookForMove(board, i, n, kingY - 1, kingX + 1, board[i][n], opponent, king);
                    threat[1] = lookForMove(board, i, n, kingY, kingX + 1, board[i][n], opponent, king);
                    threat[2] = lookForMove(board, i, n, kingY + 1, kingX + 1, board[i][n], opponent, king);
                    threat[3] = lookForMove(board, i, n, kingY + 1, kingX, board[i][n], opponent, king);
                    threat[4] = lookForMove(board, i, n, kingY + 1, kingX - 1, board[i][n], opponent, king);
                    threat[5] = lookForMove(board, i, n, kingY, kingX - 1, board[i][n], opponent, king);
                    threat[6] = lookForMove(board, i, n, kingY - 1, kingX - 1, board[i][n], opponent, king);
                    threat[7] = lookForMove(board, i, n, kingY - 1, kingX, board[i][n], opponent, king);
                    for (int y = 0; y < 8; y++)
    ...
    if (is_Safe[0] == true && board[kingY - 1][kingX + 1] == ' ' && board[kingY - 1][kingX + 1] != (queen || bishhop || knight || rook || pawn)) checkmate = false;
    if (is_Safe[1] == true && board[kingY][kingX + 1] == ' ' && board[kingY][kingX + 1] != (queen || bishhop || knight || rook || pawn)) checkmate = false;
    if (is_Safe[2] == true && board[kingY + 1][kingX + 1] == ' ' && board[kingY + 1][kingX + 1] != (queen || bishhop || knight || rook || pawn)) checkmate = false;
    if (is_Safe[3] == true && board[kingY + 1][kingX] == ' ' && board[kingY + 1][kingX] != (queen || bishhop || knight || rook || pawn)) checkmate = false;
    if (is_Safe[4] == true && board[kingY + 1][kingX - 1] == ' ' && board[kingY + 1][kingX - 1] != (queen || bishhop || knight || rook || pawn)) checkmate = false;
    if (is_Safe[5] == true && board[kingY][kingX - 1] == ' ' && board[kingY][kingX - 1] != (queen || bishhop || knight || rook || pawn)) checkmate = false;
    if (is_Safe[6] == true && board[kingY - 1][kingX - 1] == ' ' && board[kingY - 1][kingX - 1] != (queen || bishhop || knight || rook || pawn)) checkmate = false;
    if (is_Safe[7] == true && board[kingY - 1][kingX] == ' ' && board[kingY - 1][kingX] != (queen || bishhop || knight || rook || pawn)) checkmate = false;
...
void castling(bool playerTurn,       char board[8][8], 
    ...
    if (board[0][4] == king && board[0][7] == rook && playerTurn == true)
        if (board[0][5] == ' ' && board[0][6] == ' ') *ptr_shortCastp1 = true;

    // long castling, white.
    if (board[0][4] == king && board[0][0] == rook && playerTurn == true)
        if (board[0][3] == ' ' && board[0][2] == ' ' && board[0][1] == ' ') *ptr_longCastp1 = true;
    ...
    if (board[7][4] == king && board[7][0] == rook && playerTurn == false)
        if (board[7][3] == ' ' && board[7][2] == ' ' && board[7][1] == ' ') *ptr_longCastp2 = true;
...
void drawBoard(SDL_Renderer* rend_Board, SDL_Rect rect_Board[64], 
	...
	for (int i = 0; i < 64; i++) {
        ...
        // Color the squares depending on AI action.
        if (AI_running == true)
        {
            if(aiSel == i || aiMove == i)
                SDL_SetRenderDrawColor(rend_Board, 0, 0, 255, SDL_ALPHA_OPAQUE);
        }
...
void AI(char board[8][8], 
        bool shortCast,       bool longCast, 
        bool* ptr_playerTurn, bool check, 
        int kingY,            int kingX, 
        int* ptr_aiSel,       int* ptr_aiMove) {
    
    int whitePieces[16][2];
    int blackPieces[16][2];
    int white_counter = 0, black_counter = 0;
    int kingThreatY = NO_SEL, kingThreatX = NO_SEL;
    int selectY = NO_SEL, selectX = NO_SEL;
    int moveY = NO_SEL, moveX = NO_SEL;

    char piece = ' ', target = ' '; 

    AI_locateAllPieces(board, whitePieces, blackPieces, &white_counter, &black_counter);
    
    if(check == true) {
        
        AI_kingFindThreat(board, whitePieces, white_counter, kingY, kingX, &kingThreatY, &kingThreatX);
        
        if(AI_kingTryDestroyThreat(board, kingThreatY, kingThreatX, blackPieces, black_counter,      
                                   &selectY, &selectX, &moveY, &moveX) == true) 
            goto executeMove;

        if(AI_moveKing(board,  kingY, kingX, whitePieces, white_counter, &selectY, &selectX,
                       &moveY, &moveX) == true) 
            goto executeMove;
        
        if(AI_blockThreat(board, kingY, kingX, kingThreatY, kingThreatX, blackPieces, black_counter, 
                          &selectY, &selectX, &moveY, &moveX) == true)
            goto executeMove;
    }

    /*
    if(AI_evaluteThreat(board, whitePieces, white_counter, blackPieces, black_counter) == false)
        AI_pickPiece(board, blackPieces, black_counter);
    ...
    */
    ...
    AI_makeMove(board, ptr_playerTurn, selectY, selectX, moveY, moveX);
    if(AI_checkStatus(board, kingY, kingX, whitePieces, white_counter) == true) {
        AI_resetBoard(board, piece, target, selectY, selectX, moveY, moveX);
    }
    return;
}

void AI_locateAllPieces(char board[8][8], 
                        int whitePieces[16][2], int blackPieces[16][2], 
                        int* ptr_white_counter, int* ptr_black_counter) {
    
    // Fetch information about all pieces currently on the chess board.
    // Store data in one array for p1(white) and one for p2(black). 
    for(int i = 0; i < 8; i++) {
        for(int n = 0; n < 8; n++) {
            if(board[i][n] == 'P' || board[i][n] == 'R' || board[i][n] == 'K' || 
               board[i][n] == 'B' || board[i][n] == 'Q' || board[i][n] == 'W') {
                    whitePieces[*ptr_white_counter][0] = i;
                    whitePieces[*ptr_white_counter][1] = n;
                    *ptr_white_counter += 1;
            }
            if(board[i][n] == 'p' || board[i][n] == 'r' || board[i][n] == 'k' || 
               board[i][n] == 'b' || board[i][n] == 'q' || board[i][n] == 'w') {
                    blackPieces[*ptr_black_counter][0] = i;
                    blackPieces[*ptr_black_counter][1] = n;
                    *ptr_black_counter += 1;
            }
        }
    }
...
void AI_kingFindThreat(char board[8][8], 
                   int white_pieces[16][2], int white_counter, 
                   int kingY,               int kingX,
                   int* ptr_kingThreatY,    int* ptr_kingThreatX) {
   

    // iterate the array storing white pieces and try to find the threat.
    // When found dereference two pointers and add the value (X and Y) of the threat.    
    for(int i = 0; i < white_counter; i++) {
        if(lookForMove(board, white_pieces[i][0], white_pieces[i][1], kingY, kingX, 
        board[white_pieces[i][0]][white_pieces[i][1]], true, board[kingY][kingX]) == true) {
            *ptr_kingThreatY = white_pieces[i][0];
            *ptr_kingThreatX = white_pieces[i][1]; 
            break;
        }
    }
...
bool AI_kingTryDestroyThreat(char board[8][8], 
                         int kingThreatY,        int kingThreatX,
                         int blackPieces[16][2], int black_counter,
                         int* ptr_selectY,       int* ptr_selectX,
                         int* ptr_moveY,         int* ptr_moveX) {
    
    // Will try to remove identified threat, if this function is recalled the same target won't be selected.
    for(int i = 0; i < black_counter; i++){
        if(lookForMove(board, blackPieces[i][0], blackPieces[i][1], kingThreatY, kingThreatX,
        board[blackPieces[i][1]][blackPieces[i][1]], false, board[kingThreatY][kingThreatX]) == true){
            if(*ptr_selectY != blackPieces[i][0] && *ptr_selectX != blackPieces[i][0]) {
                *ptr_selectY = blackPieces[i][0];
                *ptr_selectX = blackPieces[i][1];
                *ptr_moveY = kingThreatY;
                *ptr_moveX = kingThreatX;
            }
            return true; 
            break;
        }
...
bool AI_moveKing(char board[8][8], 
                 int kingY,              int kingX,
                 int whitePieces[16][2], int white_counter,
                 int* ptr_selectY,       int* ptr_selectX, 
                 int* ptr_moveY,         int* ptr_moveX) {

    // Add data about the kings surroundings. 
    int y = NO_SEL, x = NO_SEL;
    int surroundings[8][2] = {kingY - 1, kingX - 1,
                              kingY - 1, kingX,
                              kingY - 1, kingX + 1,
                              kingY, kingX + 1,
                              kingY + 1, kingX + 1,
    ...
    // If any part of the surrounding is safe try to make a move.
    for(int i = 0; i < 8; i++) {
        for(int n = 0; n < white_counter; n++) {
            if(lookForMove(board, whitePieces[n][0], whitePieces[n][1], surroundings[i][0], surroundings[i][1], 
                        board[whitePieces[n][0]][whitePieces[n][1]], true, board[kingY][kingX]) == true) break;

            if(n == white_counter) {
                y = surroundings[i][0];
                x = surroundings[i][1];
                if(board[y][x] != 'p' && board[y][x] != 'r' && board[y][x] != 'k' && 
                   board[y][x] != 'b' && board[y][x] != 'q') {
                       *ptr_moveY = y;
                       *ptr_moveX = x;
                       *ptr_selectY = kingY;
                       *ptr_selectX = kingX;
...
void AI_makeMove(char board[8][8], bool* ptr_playerTurn,
                 int selectY,      int selectX,
                 int moveY,        int moveX) {
...
bool AI_checkStatus(char board[8][8],        
                    int kingY,              int kingX, 
                    int whitePieces[16][2], int white_counter) {

    // At the end of computer turn, check kings check status.
    for(int i = 0; i < white_counter; i++) {
        if(lookForMove(board, whitePieces[i][0], whitePieces[i][1], kingY, kingX, 
                       board[whitePieces[i][0]][whitePieces[i][1]], true, board[kingY][kingX]) == true)
            return false;
    }
    return true; 
}

void AI_resetBoard(char board[8][8],
                   char piece,  char target,
                   int selectY, int selextX,
                   int moveY,   int moveX) {
    
    // If computer move is wrong, reset turn.
    board[selectY][selextX] = piece;
    board[moveY][moveX] = target;

    return;
}

bool AI_blockThreat(char board[8][8],
                    int kingY,              int kingX,
                    int kingThreatY,        int kingThreatX,
                    int blackieces[16][2],  int black_counter,
                    int* ptr_selectY,       int* ptr_selectX,
                    int* ptr_moveY,         int* ptr_moveX) {
    ...
    if(board[kingThreatY][kingThreatX] == 'K') return false; 
    
    // Rook and Queen threat: check for a possiblity block thier path (vertical and horizontal).
    if(kingThreatY == kingY) {
        if (kingThreatX > kingX) {
            for(int i = 0; i < black_counter; i++) {
                for(int n = kingX; n < kingThreatX; n++) {
                    if(lookForMove(board, blackieces[i][0], blackieces[i][1], kingThreatY, n,
                    board[blackieces[i][0]][blackieces[i][1]], false, board[kingThreatY][n]) == true) {
                        *ptr_selectY = blackieces[i][0];
                        *ptr_selectX = blackieces[i][1];
                        *ptr_moveY = kingThreatY;
                        *ptr_moveX = n;
                        return true; 
                    }
            ...
            for(int i = 0; i < black_counter; i++) {
                for(int n = kingThreatX; n < kingX; n++) {
                    if(lookForMove(board, blackieces[i][0], blackieces[i][1], kingThreatY, n,
                    board[blackieces[i][0]][blackieces[i][1]], false, board[kingThreatY][n]) == true) {
                        *ptr_selectY = blackieces[i][0];
                        *ptr_selectX = blackieces[i][1];
    ...
    if(kingThreatX == kingX) { 
        if (kingThreatY > kingY) {
            for(int i = 0; i < black_counter; i++) {
                for(int n = kingY; n < kingThreatY; n++) {
                    if(lookForMove(board, blackieces[i][0], blackieces[i][1], n, kingThreatX,
                    board[blackieces[i][0]][blackieces[i][1]], false, board[n][kingThreatX]) == true) {
                        *ptr_selectY = blackieces[i][0];
                        *ptr_selectX = blackieces[i][1];
                        *ptr_moveY = n;
                        *ptr_moveX = kingThreatX;
            ...
            for(int i = 0; i < black_counter; i++) {
                for(int n = kingThreatY; n < kingY; n++) {
                    if(lookForMove(board, blackieces[i][0], blackieces[i][1], n, kingThreatX,
                    board[blackieces[i][0]][blackieces[i][1]], false, board[kingThreatY][n]) == true) {
                        *ptr_selectY = blackieces[i][0];
                        *ptr_selectX = blackieces[i][1];
    ...
    // Queen or Bishop threat: check for a possiblity block thier path (Diagonal).
    if(kingX != kingThreatX && kingY != kingThreatY) {
        
        trackX = kingX;
        if(kingThreatY > kingY && kingThreatX < kingX) { // Left down.
            for(int i = kingY; i < kingThreatY; i++)
                for (int n = trackX; n > kingThreatX; n--) {
                    trackX = n;
                    for(int k = 0; k < black_counter; k++) {
                        if(lookForMove(board, blackieces[k][0], blackieces[k][1], i, trackX, 
                        board[blackieces[k][0]][blackieces[k][1]], false, board[i][trackX]) == true) {
                            *ptr_selectY = blackieces[i][0];
                            *ptr_selectX = blackieces[i][1];
                            *ptr_moveY = n;
                            *ptr_moveX = trackX;
        ...
        trackX = kingX;
        if(kingThreatY > kingY && kingThreatX > kingX) {
            for(int i = kingY; i < kingThreatY; i++)
                for (int n = trackX; n < kingThreatX; n++) {
                    trackX = n;
                    for(int k = 0; k < black_counter; k++) {
...
void AI_markSquares(int y_location, int x_location, int* ptr_square) {

    // Graphical function, Ai will tell about its move. 
    int counter = 0; 
    for (int i = 0; i < 8; i++) {
        for (int n = 0; n < 8; n++) {
            if (i == y_location && n == x_location) {
                *ptr_square = counter; 
                return;
            }
            counter++;
